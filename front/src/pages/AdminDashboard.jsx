...
export default function AdminDashboard() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f4eee4] px-4 pb-32 pt-28 text-[#20150f] sm:px-6 md:px-10">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#a86b00]">Control room</p><h1 className="mt-2 font-display text-4xl sm:text-5xl">Your library</h1><p className="mt-2 text-sm text-[#20150f]/60">Upload, publish, and curate every track.</p></div><button onClick={logout} className="rounded-full border border-[#20150f]/15 px-4 py-2 text-xs hover:border-[#a86b00]">Sign out</button></div>
        ...
        <div className="mt-8 grid min-w-0 gap-6 lg:grid-cols-[minmax(320px,0.8fr)_minmax(0,1.2fr)]"><UploadBox token={token} onCreated={(song) => setSongs((items) => [song, ...items])} /><section className="min-w-0 rounded-2xl border border-[#20150f]/10 bg-white/55 p-5 shadow-lg backdrop-blur-md md:p-7"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs uppercase tracking-[0.2em] text-[#20150f]/45">Catalogue</p><h2 className="mt-1 font-display text-2xl">{songs.length} tracks</h2></div><button onClick={() => loadSongs()} className="h-9 w-9 flex-shrink-0 rounded-full border border-[#20150f]/15 hover:border-[#a86b00]" aria-label="Refresh songs"><i className="ri-refresh-line" /></button></div>{loading ? <p className="py-10 text-center text-sm text-[#20150f]/50">Loading catalogue...</p> : <div className="mt-5 divide-y divide-[#20150f]/10">{songs.map((song) => <article key={song.id} className="flex flex-wrap items-center gap-3 py-3"><div className="h-11 w-11 flex-shrink-0 overflow-hidden rounded-lg bg-[#20150f]/10">{song.coverImage && <img src={song.coverImage} alt="" className="h-full w-full object-cover" />}</div><div className="min-w-0 flex-1 basis-[calc(100%-3.75rem)] sm:basis-0"><p className="truncate text-sm font-semibold">{song.title}</p><p className="truncate text-xs text-[#20150f]/55">{song.artist}{song.album ? ` · ${song.album}` : ''}</p></div><button onClick={() => update(song.id, { isPublished: !song.isPublished })} className={`ml-auto rounded-full px-3 py-1 text-[11px] ${song.isPublished ? 'bg-emerald-100 text-emerald-800' : 'bg-[#20150f]/10 text-[#20150f]/60'}`}>{song.isPublished ? 'Published' : 'Draft'}</button><button onClick={() => remove(song)} className="h-8 w-8 flex-shrink-0 rounded-full text-red-700 hover:bg-red-100" aria-label={`Delete ${song.title}`}><i className="ri-delete-bin-6-line" /></button></article>)}</div>}</section></div>
      </div>
    </main>
  );
}